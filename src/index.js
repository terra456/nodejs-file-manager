...
argv.forEach((el, i) => {
  if (el.startsWith('--') && el.includes('=')) {
    ...
    process.stdout.write(`You are currently in ${folder}${os.EOL}`);
  }
});

const fileSystem = new FileSystem(folder, os.EOL);
...
process.stdin.on('data', async (data) => {
  ...
  try {
    ...
    process.stdout.write(`You are currently in ${fileSystem.curent}${os.EOL}`);
  }
});
